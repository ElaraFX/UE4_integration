...
#if WITH_EDITOR
int32 UMaterialExpressionCustomMultiOut::Compile(class FMaterialCompiler* Compiler, int32 OutputIndex, int32 MultiplexIndex)
{
...
void SetMask(FExpressionOutput& output, int32 UseMask, int32 R, int32 G, int32 B, int32 A)
{
	output.Mask = UseMask;
	output.MaskR = R;
	output.MaskG = G;
...
bool UMaterialExpressionCustomMultiOut::DoImportShader(int32 currentShaderIndex)
{
	...
	TSharedPtr<class IMaterialEditor> pMaterialEditor = ::GetIMaterialEditorForObject(this);
	...
	while (parameterMatcher.FindNext())
	{
		...
		else if (type.StartsWith("float"))
		{
			for (int32 i = 0; i < shaderInfo.MaxShaderCount; ++i)
			{
				...
				if (NULL != pVectorExpression)
				{
					...
					{
						SetMask(pVectorExpression->Outputs[0], 1, 1, 1, 1, 0);
					}
					else if (type == TEXT("float2"))
					{
						SetMask(pVectorExpression->Outputs[0], 1, 1, 1, 0, 0);
					}
		...
		{
			static UClass* textureClass = NULL;
			if (NULL == textureClass)
			{
				for (TObjectIterator<UMaterialExpressionTextureSampleParameter> It; It; ++It)
				{
					UMaterialExpressionTextureSampleParameter* pTexutreObjectParameter = *It;
					if (NULL != pTexutreObjectParameter && pTexutreObjectParameter->GetClass()->GetName() == TEXT("MaterialExpressionTextureObjectParameter"))
					{
						textureClass = pTexutreObjectParameter->GetClass();
						break;
					}
				}
			}
			...
			for (int32 i = 0; i < shaderInfo.MaxShaderCount; ++i)
			{
				...
				if (NULL != textureClass)
				{
					UMaterialExpressionTextureObjectParameter* pTextureExpression = static_cast<UMaterialExpressionTextureObjectParameter*>(pMaterialEditor->CreateNewMaterialExpression(
						textureClass, localPos, false, true));
